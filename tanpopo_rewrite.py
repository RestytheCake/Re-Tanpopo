...
class AnimeViewer:
    def __init__(self, master):
        ...
        self.auth_button = CTkButton(self.button_frame, text="Refresh Authentication",
                                     font=CTkFont(size=12, weight="bold"), command=ToplevelWindow)
        ...
        username = ls.getItem("username")
        if username is None:
            self.username_label = CTkLabel(self.avatar_frame, text=f"Log In", fg_color=grey(), padx=10,
        ...
        self.recently_text = CTkLabel(master=self.watching_frame, text="Continue Watching",
                                      font=CTkFont(size=18, weight="bold"), text_color="white", fg_color=grey())
        self.recently_text.grid(row=0, column=0, padx=5, sticky="nw")

        ## Watching Shows
        self.recently_video_frame = CTkScrollableFrame(master=self.watching_frame, fg_color=grey(), orientation="horizontal", height=120)
        ...
        for i in range(len(self.recently_updated_urls)):
            shimmer_label = create_shimmer_label(self.recently_video_frame, 80, 115)
            shimmer_label.grid(row=0, column=i, padx=10)
            self.recently_updated_shimmer_labels.append(shimmer_label)

        threading.Thread(target=update_ui_with_images, args=(
            self.recently_updated_urls, self.recently_video_frame, (80, 115),
            self.recently_updated_shimmer_labels)).start()
        ### threading for Python Multitasking, so that python doesn't lag when loading the images with poor WIFI or bad server
        ...
        version_text = CTkLabel(self.master, text="ver 0.0.7", text_color="#FFFFFF", fg_color="#121212", padx="10")
