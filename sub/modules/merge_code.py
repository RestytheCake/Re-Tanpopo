...
def merge_files(source_dir, output_file, include_dirs, script_name):
    ...
    with open(output_file, 'w', encoding='utf-8') as outfile:
        ...
        for root, dirs, files in os.walk(source_dir):
            ...
            if any(os.path.commonpath([root, os.path.join(source_dir, d)]) == root for d in include_dirs):
                # Process files
...
if __name__ == "__main__":
    ...
    include_directories = ['sub']  # Add other directories to include if needed

    # Define the path for the output file
    output_file_path = os.path.join(source_directory, output_directory, output_filename)

    # Run the merge operation
    merge_files(source_directory, output_file_path, include_directories, script_name)
